...
public class DashboardUI extends JFrame {
	...
	public DashboardUI() {
		...
		billTable.getColumn("Edit")
				.setCellEditor(new ButtonEditor(new JCheckBox(), "edit", "✏️", new Color(0, 200, 0)));

		billTable.getColumn("Delete").setCellRenderer(new ButtonRenderer("🗑", new Color(255, 69, 58)));
		billTable.getColumn("Delete")
				.setCellEditor(new ButtonEditor(new JCheckBox(), "delete", "🗑", new Color(255, 69, 58)));

		billTable.getColumn("Menu").setCellRenderer(new ButtonRenderer("⋮", new Color(200, 200, 200)));
		billTable.getColumn("Menu")
				.setCellEditor(new ButtonEditor(new JCheckBox(), "menu", "⋮", new Color(200, 200, 200)));
		...
		createBillBtn.addActionListener(e -> {
			BillUI billFrame = new BillUI();

			billFrame.addWindowListener(new WindowAdapter() {
				@Override
				public void windowClosed(WindowEvent we) {
					refreshTable(); // → tableModel cleared + loadBillsFromDatabase()
				}
			});
		});
	...
	private void loadBillsFromDatabase() {
		try (Connection conn = com.billing.DBConnection.getConnection()) {
			...
			while (rs.next()) {
				...
				tableModel.addRow(
						new Object[] { billId, billNo, name, date.toString(), "\u20B9" + total, "✏️", "🗑", "︙" });
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	class ButtonEditor extends DefaultCellEditor {
		...
		public Object getCellEditorValue() {
			...
			if (action.equals("edit")) {
				...
				view.addActionListener(e -> new ViewBillUI(billid));
				download.addActionListener(e -> {
				    try (Connection conn = com.billing.DBConnection.getConnection()) {

				        PreparedStatement psHeader = conn.prepareStatement(
				            "SELECT bill_no, customer_name, customer_address, bill_date, total_amount "
				          + "FROM bills WHERE bill_id = ?");
				        psHeader.setInt(1, billid);
				        ResultSet rsH = psHeader.executeQuery();

				        if (!rsH.next()) {
				            JOptionPane.showMessageDialog(null,
				                    "No bill found for ID " + billid, "Error", JOptionPane.ERROR_MESSAGE);
				            return;
				        }

				        String billNo        = rsH.getString("bill_no");
				        String customerName  = rsH.getString("customer_name");
				        String customerAddr  = rsH.getString("customer_address");
				        String billDate      = rsH.getDate  ("bill_date").toString(); // yyyy‑MM‑dd
				        int    totalAmount   = rsH.getInt   ("total_amount");

				        PreparedStatement psItems = conn.prepareStatement(
				            "SELECT particulars, rate, quantity, amount "
				        ...
				        File pdf = PDFGenerator.generateBillPDF(
				                billNo, customerName, customerAddr,
				                billDate, totalAmount, tempModel);
				        

				        if (pdf != null) {       // null means user cancelled Save dialog
				            PDFGenerator.openFile(pdf);
				        }
				        ...
				        JOptionPane.showMessageDialog(null,
				                ...
				                "Database/PDF Error", JOptionPane.ERROR_MESSAGE);
				    }
				});
